...
/**
 * Created by consult on 2017/5/28.
 */
 ...
 */
public class HVScrollViewTwo extends FrameLayout {
    private static final String TAG = "HVScrollViewTwo";
    ...
    private float mImageFourBottom;
    private float mImageThreeBottom;
    private float mImageChaBottom;
    private float mImageFourLeft;
    private float mImageThreeLeft;
    private float mImageChaLeft;
    ...
    private ImageView homeMailList;

    private ImageView homeQiuhuan;

    private ImageView homeRankingList;
    ...
    private float fromXOne, fromXTwo, fromXThree, fromXFour;

    private float fromYOne, fromYTwo, fromYThree, fromYFour;

    float scaleOne = 1, scaleTwo = 1, scaleThree = 1, scaleFour = 1;


    private Bitmap bgBitmat;

    private int bgWidth;    //背景图片的宽度

    private int bgHeitht;   //背景图片的高度


    private Bitmap bgXingxingBitmat;

    private int bgXingxingWidth;    //背景星星图片的宽度

    private int bgXingxingHeitht;   //背景星星图片的高度


    private int maxY;  //滑动到最低端的屏幕纵坐标
    ...
    float scaleTwoDegress;
    float scaleThreeDegress;
    float scaleFourDegress;

    public HVScrollViewTwo(Context context) {
        this(context, null);
    }

    public HVScrollViewTwo(Context context, AttributeSet attrs) {
        super(context, attrs);
        initScrollView();
    }
    ...
    private void initScrollView() {
        try {
            ...
            mHeidht = wm.getDefaultDisplay().getHeight();


//            bgBitmat = BitmapUtils.readRawStream(getContext(),R.raw.home_background);
            BitmapFactory.Options opt = new BitmapFactory.Options();
            opt.inSampleSize = 8;
            bgBitmat = BitmapFactory.decodeResource(getResources(),R.drawable.home_background,opt);
            bgWidth = bgBitmat.getWidth() * 8;
            bgHeitht = bgBitmat.getHeight() * 8;

            if (!bgBitmat.isRecycled()) {
                bgBitmat.recycle();
            }

            bgXingxingBitmat = BitmapFactory.decodeResource(getResources(),R.drawable.home_xingxing,opt);

            bgXingxingHeitht = bgXingxingBitmat.getHeight() * 4;
            bgXingxingWidth = bgXingxingBitmat.getWidth() * 4;

            if(!bgXingxingBitmat.isRecycled()){
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    mLastMotionX = x;
                    // 全方向滚动
                    scrollBy(deltaX / 2, deltaY / 2);
                    Log.d(TAG, "deltaY =:" + String.valueOf(deltaY) + "===============deltaX=" + String.valueOf(deltaX));
                    ImageView xingXing = (ImageView) ((FrameLayout) getChildAt(0)).getChildAt(1);

                    if ((xingXing.getLeft() <= (bgXingxingWidth - bgWidth) / 2) || (xingXing.getTop() <= (bgXingxingHeitht - bgHeitht) / 2)) {
                        xingXing.layout(0, 0,
                                xingXing.getWidth(), xingXing.getBottom());
                    } else {
                        Log.e("星星：", "左" + xingXing.getLeft() + ",上" + xingXing.getTop());
                        xingXing.layout(xingXing.getLeft() - deltaX, xingXing.getTop() - deltaY,
                                xingXing.getRight() - deltaY, xingXing.getBottom() - deltaY);
                    }

                    RelativeLayout rl = (RelativeLayout) ((FrameLayout) getChildAt(0)).getChildAt(2);
                    FrameLayout oneStarFrameLayout = (FrameLayout) rl.getChildAt(2);
                    ImageView twoStarImageView = (ImageView) rl.getChildAt(4);
                    FrameLayout threeStarFrameLayout = (FrameLayout) rl.getChildAt(3);
                    FrameLayout fourStarFrameLayout = (FrameLayout) rl.getChildAt(5);
                    if (mIsFirstCreat) {
                        mImageFourBottom = oneStarFrameLayout.getBottom();
                        mImageFourLeft = oneStarFrameLayout.getLeft();
                        mImageChaBottom = twoStarImageView.getBottom() - mImageFourBottom;
                        mImageChaLeft = twoStarImageView.getLeft() - mImageFourLeft;
                        mIsFirstCreat = false;
                    }
//                    if (getScrollY() < maxY && (fourStarFrameLayout.getBottom() - deltaY * 4 <= bgHeitht) && (fourStarFrameLayout.getBottom() - deltaY * 4 >= bgHeitht - 1400)) {
                    ...
                    if (twoStarImageView.getBottom() - deltaY - oneStarFrameLayout.getBottom() > mImageChaBottom || deltaY < 0) {
                        ...
                        twoStarImageView.layout(twoStarImageView.getLeft(), twoStarImageView.getTop() - deltaY * 2,
                                twoStarImageView.getRight(), twoStarImageView.getBottom() - deltaY * 2);
                        threeStarFrameLayout.layout(threeStarFrameLayout.getLeft(), threeStarFrameLayout.getTop() - deltaY * 4,
                                threeStarFrameLayout.getRight(), threeStarFrameLayout.getBottom() - deltaY * 4);
                        fourStarFrameLayout.layout(fourStarFrameLayout.getLeft(), fourStarFrameLayout.getTop() - deltaY * 6,
                                fourStarFrameLayout.getRight(), fourStarFrameLayout.getBottom() - deltaY * 6);
...
//                  int initialVelocity = (int) velocityTracker.getYVelocity(mActivePointerId);
    ...
    private void loadScaleAnim(float scaleDegress) {
        scaleTwoDegress = scaleDegress;
        scaleThreeDegress = scaleDegress;
        scaleFourDegress = scaleDegress;

        //获取星球对象
        FrameLayout oneFrameLayout = (FrameLayout) ((MainActivity) getContext()).findViewById(R.id.one_frame_layout);
        FrameLayout twoFrameLayout = (FrameLayout) ((MainActivity) getContext()).findViewById(R.id.two_frame_layout);
        ImageView threeStar = (ImageView) ((MainActivity) getContext()).findViewById(R.id.three_star);
        FrameLayout fourFrameLayout = (FrameLayout) ((MainActivity) getContext()).findViewById(R.id.four_frame_layout);
        ...
        oneFrameLayout.setAnimation(scaleOneAnimation);
        twoFrameLayout.setAnimation(scaleOneAnimation);
        threeStar.setAnimation(scaleOneAnimation);
        fourFrameLayout.setAnimation(scaleOneAnimation);
    }

    private void loadStarsAnim(boolean toInit, float deltalY) {
        ...
        //垂直滑动
        distance = 4000 - scrollY;

        float distanceY = Math.abs(deltalY);

        if (deltalY < 0) {           //放大

//            scaleOne = 1 + (float) distance / mHeidht * 8;
//            scaleTwo = 1 + (float) distance / mHeidht * 4;
//            scaleThree = 1 + (float) distance / mHeidht * 2;
            if (scaleFour < 1.5) {
                scaleFour += 0.05;
                scaleThree = scaleFour;
                scaleTwo = scaleFour;
                scaleOne = scaleFour;
            }
        } else if (deltalY != 0) {                    //缩小
            if (scaleFour > 0.8) {
                scaleFour -= 0.05;
                scaleThree = scaleFour;
                scaleTwo = scaleFour;
        ...
        Log.e("首页动画", "fromXOne=" + fromXOne + ",scaleOne=" + scaleOne);
        //创建缩放动画
        ScaleAnimation animationOne = new ScaleAnimation(fromXOne, scaleOne, fromYOne, scaleOne,
                Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
        ScaleAnimation animationTwo = new ScaleAnimation(fromXTwo, scaleTwo, fromYTwo, scaleTwo,
                Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
        ScaleAnimation animationThree = new ScaleAnimation(fromXThree, scaleThree, fromYThree, scaleThree,
                Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
        ScaleAnimation animationFour = new ScaleAnimation(fromXFour, scaleFour, fromYFour, scaleFour,
                Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
        ...
        //将动画放入动画集中
        oneFrameLayout.setAnimation(animationOne);
        twoFrameLayout.setAnimation(animationTwo);
        threeStar.setAnimation(animationThree);
        fourFrameLayout.setAnimation(animationFour);
        /** 开始动画 */
        fromXOne = scaleOne;
        fromXTwo = scaleTwo;
        fromXThree = scaleThree;
        fromXFour = scaleFour;
        fromYOne = scaleOne;
        fromYTwo = scaleTwo;
        fromYThree = scaleThree;
        fromYFour = scaleFour;
        Log.e("HVScrollView", "distance=" + distance);
    }
